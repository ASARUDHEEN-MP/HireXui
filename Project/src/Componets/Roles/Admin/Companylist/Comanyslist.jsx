...
function Comanyslist() {
  ...
  const toggleBlock = (userId, isBlocked) => {
    instance
      .patch(`admin/company-manage/${userId}/`, { is_blocked: isBlocked })
      .then(() => {
        setUsers(
          users.map((user) =>
  ...
  const deleteUser = async () => {
    try {
      handleClose();
      instance.delete(`admin/company-manage/${userId}/`);
          ...
          {filteredUserList
            ...
            .map((user, index) => (
              <tr key={index}>
                ...
                <td>{user.username}</td>
                <td>{user.email}</td>
                <td>{user.is_active ? 'Active' : 'Not Active'}</td> {/* Display 'Active' or 'Not Active' */}
                <td>
                  <Button
